...
### 13.4 지금 상태 (redesign · 2026-09-07)
- `index.html` 0.88MB · 항목번호 151개 · 8개 화면 부팅 오류 0 · `npm run verify` 전부 통과
...
- 이번 세션에서 더한 것 —
  ...
  **2026 당직매뉴얼**(hwpx→md→전문 표출·AI 배정 근거·용역업체 번호) · 민원검색 목록 좁히기
- 고친 중대 하자 —
  ① 완료 민원이 폴링에 되살아나던 것(3.10) ② 부서 검색 한글 조합 깨짐(3.11)
  ③ 없는 칸 때문에 저장 전체가 실패하던 것(3.13) ④ 접수번호 재배정 시 지시-민원 끈이 끊기던 것(3.14)
  ⑤ 처리부서 지정 결과가 저장 키 불일치로 다시 읽히지 않던 것(3.16)

### 13.5 아직 안 한 것
- **실기기 확인** — jsdom 은 레이아웃을 계산하지 않는다. 겹침·줄바꿈·색 대비는 브라우저에서 봐야 한다.
  이번 세션의 화면 변경(매뉴얼 표·검색 안내 줄·연락처 구획)이 특히 그렇다.
- `redesign` → `main` 병합(배포). 프리뷰로 충분히 본 뒤에 한다.
- 매뉴얼에서 더 캘 것 — 용역업체는 지금 1곳만 자동 추출된다(찻길 동물 사체).
  우수관로·제설 등 표로만 적힌 업체는 `scripts/md2manual.py` 의 `vendor_of` 를 넓히면 된다.
